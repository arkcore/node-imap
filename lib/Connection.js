...
function Connection(config) {
  ...
  if (config.socks) {
    this._config.socksHost = config.socks.host;
    this._config.socksPort = config.socks.port;
  }
...
function tlsConnection(socksSocket, options) {
    var handleSocksConnectToHost;

    handleSocksConnectToHost = socksSocket.handleSocksConnectToHost;
    socksSocket.handleSocksConnectToHost = function() {
        var verifyHost;

        if (options.tlsOptions && options.tlsOptions.rejectUnauthorized !== false) {
...
Connection.prototype.connect = function() {
  ...
  if (config.socksHost || config.socksPort) {
    socket = socksClient.createConnection(config);
    if (config.tls)
        socket = tlsConnection(socket, config);
  } else {
    socket = new Socket();
  }
